...
from wasserstein_filter.filters import wasserstein_filter_sqrt
from wasserstein_filter.models.markov_stochastic_volatility_sqrt import (
    build_model,
    generate_data,
)
...
plt.fill_between(
    t,
    filt_states_mean[:, 0] - 2.0 * filt_states_cov_sqrt[:, 0, 0],
    filt_states_mean[:, 0] + 2.0 * filt_states_cov_sqrt[:, 0, 0],
    color="tab:red",
    alpha=0.25,
)
